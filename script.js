...
document.getElementById('convertBtn').addEventListener('click', async () => {
    ...
    feedbackDiv.innerHTML = `<p><strong class="has-text-primary">${baseCurrency}</strong> priced at <strong class="has-text-primary">${CURRENCY_ICONS[baseCurrency]} ${baseValue || 1.00}</strong> converts to the following</p>`;
    ...
    for (const [currency, rate] of Object.entries(data.data)) {
        ...
        if (baseValue) {
            ...
            row.innerHTML = `<td>${currency}</td><td><strong class='has-text-primary value-sign'>${CURRENCY_ICONS[currency]}</strong>${convertedValue.toFixed(2)}</td>`;
        }
        else {
            row.innerHTML = `<td>${currency}</td><td><strong class='has-text-primary value-sign'>${CURRENCY_ICONS[currency]}</strong>${rate.toFixed(2)}</td>`;
        }
